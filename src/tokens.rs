...
static OPERATORS: &[Operator] = &[
    ...
    Operator { kind: OperatorType::Mod, repr: &["%", "mod"], precedence: 3, associativity: Associativity::Left, arity: 2, doit: |arr| arr[0] % arr[1] },
    Operator { kind: OperatorType::Sin, repr: &["sin"], precedence: 4, associativity: Associativity::Right, arity: 1, doit: |arr| arr[0].sin() },
...
impl Operator {
    ...
    pub fn by_repr(repr: &str) -> Option<(&'static Operator, &'static &'static str)> {
        OPERATORS.iter().find_map(|op|
            op.repr.iter().find(|str| {
                repr.to_lowercase().starts_with(&str.to_lowercase().to_string())
...
static CONSTANTS: &[Constant] = &[
    Constant { kind: ConstantType::PI, repr: &["π", "pi"], value: std::f64::consts::PI },
    Constant { kind: ConstantType::Tau, repr: &["τ", "tau"], value: std::f64::consts::TAU },
    Constant { kind: ConstantType::E, repr: &["e"], value: std::f64::consts::E }
];
